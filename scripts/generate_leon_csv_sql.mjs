...
// Build a lookup: road name (lowercased, normalized) -> barangay
...
let sql = '-- ============================================================\n';
sql += '-- KalsaTrack - LGU Leon Barangay Roads Inventory (' + roads.length + ' roads)\n';
sql += '-- Generated from Leon_barangay_roads.csv with barangay mapping\n';
sql += '-- Run this in Supabase SQL Editor\n';
sql += '-- ============================================================\n\n';

sql += 'CREATE TABLE IF NOT EXISTS public.lgu_road_inventory (\n';
sql += '  id                    BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,\n';
sql += '  road_name             TEXT NOT NULL,\n';
sql += '  barangay              TEXT DEFAULT '''',\n';
sql += '  municipality          TEXT DEFAULT ''Leon'',\n';
sql += '  province              TEXT DEFAULT ''Iloilo'',\n';
sql += '  classification        TEXT DEFAULT '''',\n';
sql += '  length_km             DOUBLE PRECISION DEFAULT 0,\n';
sql += '  right_of_way_m        DOUBLE PRECISION DEFAULT 0,\n';
sql += '  year_constructed      INTEGER,\n';
sql += '  concrete_km           DOUBLE PRECISION,\n';
sql += '  concrete_pct          DOUBLE PRECISION,\n';
sql += '  concrete_condition    TEXT,\n';
sql += '  asphalt_km            DOUBLE PRECISION,\n';
sql += '  asphalt_pct           DOUBLE PRECISION,\n';
sql += '  asphalt_condition     TEXT,\n';
sql += '  gravel_km             DOUBLE PRECISION,\n';
sql += '  gravel_pct            DOUBLE PRECISION,\n';
sql += '  gravel_condition      TEXT,\n';
sql += '  earth_km              DOUBLE PRECISION,\n';
sql += '  earth_pct             DOUBLE PRECISION,\n';
sql += '  earth_condition       TEXT,\n';
sql += '  source                TEXT DEFAULT ''LGU Leon Barangay Roads CSV'',\n';
sql += '  created_at            TIMESTAMPTZ DEFAULT now(),\n';
sql += '  updated_at            TIMESTAMPTZ DEFAULT now()\n';
sql += ');\n\n';

sql += 'ALTER TABLE public.lgu_road_inventory ENABLE ROW LEVEL SECURITY;\n\n';

sql += 'CREATE POLICY IF NOT EXISTS "auth_sel_lgu_road_inventory"\n';
sql += '  ON public.lgu_road_inventory FOR SELECT TO authenticated USING (true);\n';
sql += 'CREATE POLICY IF NOT EXISTS "anon_sel_lgu_road_inventory"\n';
sql += '  ON public.lgu_road_inventory FOR SELECT TO anon USING (true);\n';
sql += 'CREATE POLICY IF NOT EXISTS "auth_ins_lgu_road_inventory"\n';
sql += '  ON public.lgu_road_inventory FOR INSERT TO authenticated WITH CHECK (true);\n';
sql += 'CREATE POLICY IF NOT EXISTS "auth_upd_lgu_road_inventory"\n';
sql += '  ON public.lgu_road_inventory FOR UPDATE TO authenticated USING (true);\n';
sql += 'CREATE POLICY IF NOT EXISTS "auth_del_lgu_road_inventory"\n';
sql += '  ON public.lgu_road_inventory FOR DELETE TO authenticated USING (true);\n\n';

sql += 'DO $$\n';
sql += 'BEGIN\n';
sql += '  IF NOT EXISTS (\n';
sql += '    SELECT 1 FROM pg_publication_tables\n';
sql += '    WHERE pubname = ''supabase_realtime'' AND tablename = ''lgu_road_inventory''\n';
sql += '  ) THEN\n';
sql += '    ALTER PUBLICATION supabase_realtime ADD TABLE public.lgu_road_inventory;\n';
sql += '  END IF;\n';
sql += 'END $$;\n\n';

sql += 'INSERT INTO public.lgu_road_inventory (\n';
sql += '  road_name, barangay, municipality, classification, length_km,\n';
sql += '  right_of_way_m, year_constructed,\n';
sql += '  concrete_km, concrete_pct, concrete_condition,\n';
sql += '  asphalt_km, asphalt_pct, asphalt_condition,\n';
sql += '  gravel_km, gravel_pct, gravel_condition,\n';
sql += '  earth_km, earth_pct, earth_condition\n';
sql += ') VALUES\n';

const vals = roads.map(function(r) {
  return '(' + q(r.name) + ', ' + q(r.barangay) + ", 'Leon', 'Iloilo', " + q(r.class) + ', ' + r.length + ', ' + r.row + ', ' + r.year + ', ' + r.conL + ', ' + r.conPct + ', ' + r.conC + ', ' + r.aspL + ', ' + r.aspPct + ', ' + r.aspC + ', ' + r.grvL + ', ' + r.grvPct + ', ' + r.grvC + ', ' + r.ethL + ', ' + r.ethPct + ', ' + r.ethC + ')';
});
...
sql += '\n-- ============================================================\n';
sql += '-- Done! ' + roads.length + ' roads, ' + totalKm.toFixed(2) + ' km total\n';
sql += '-- Barangays matched: ' + matched + '/' + roads.length + '\n';
sql += '-- ============================================================\n';
